...
@section('title', 'Halaman Galleri - Video ' . Str::ucfirst(request()->status))
...
@section('content')
    ...
    <section class="content">
        <div class="card">
            <div class="card-body">
                <p>Setiap Anda menambahkan album video baru atau melakukan perubahan pada album video status terbit akan dipindahkan
                ...
                <div class="table-responsive mt-3">
                    <table class="table table-bordered table-hover dataTable dtr-inline" id="tbgalleryvideo"
                        style="width: 100%">
                        <thead class="bg-green">
                            <tr>
                                <th><input type="checkbox" class="checkbox_all"></th>
                                <th>NAMA ALBUM VIDEO</th>
                                <th>DIBUAT OLEH</th>
                                <th>STATUS</th>
                                <th>DIPERBAHARUI</th>
                                <th>JUMLAH VIDEO</th>
...
@section('js')
    ...
    <script>
        $(document).ready(function() {
            $('#tbgalleryvideo').DataTable({
                processing: true,
                serverSide: true,
                responsive: true,
                ajax: {
                    url: "{{ url('backend/gallery/video/read') }}",
                    type: 'GET',
                    data: {
                        '_token': "{{ csrf_token() }}",
            ...
            $('#update_status').click(function() {
                ...
                if (id.length > 0) {
                    Swal.fire({
                        title: 'Konfirmasi persetujuan status gallery album video',
                        input: 'select',
                        inputOptions: {
                            'Terbit': 'Terbit',
                            'Arsip': 'Arsip'
                        ...
                        inputValidator: function(value) {
                            return new Promise(function(resolve, reject) {
                                if (value == '') {
                                    resolve(
                                        'Anda harus memilih persetujuan status gallery album video');
                                } else {
                                    resolve();
                                }
                            });
                        ...
                        if (result.isConfirmed) {
                            $.ajax({
                                ...
                                success: function(data) {
                                    ...
                                    $('#tbgalleryvideo').DataTable().ajax.reload();
                                },
                                error: function(data) {
                                    Swal.fire({
                                        title: "Gagal!",
            ...
            $('#add').click(function() {
                Swal.fire({
                    title: 'Tambah Album Video',
                    input: 'text',
                    inputPlaceholder: 'Nama Album Video',
                    showCancelButton: true,
                    ...
                    inputValidator: function(value) {
                        return new Promise(function(resolve, reject) {
                            if (value == '') {
                                resolve(
                                    'Nama Album Video tidak boleh kosong!');
                            } else {
                                resolve();
                            }
                        });
